...
//
//#define RELOC_FLAG32 (RelInfo) ((RelInfo >> 0x0C) == IMAGE_REL_BASSED_HIGHLOW)
//#define RELOC_FLAG64 (RelInfo) ((RelInfo >> 0x0C) == IMAGE_REL_BASSED_DIR64)
//
//#ifdef _WIN64
...
void __stdcall ShellCode(MANUAL_MAPPING_DATA* pData)
{
	...
	auto _DllMain = reinterpret_cast<f_DLL_ENTYRY_POINT>(pBase + pOpt->AddressOfEntryPoint);
	...
	if (LocationDelta)
	{
		...
		{
			UINT AmoutOfEntries = (pRelocData->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(WORD);
			WORD* pRelativeInfo = reinterpret_cast<WORD*>(pRelocData + 1);
			for (UINT i = 0; i != AmoutOfEntries; ++i, ++pRelativeInfo)
			{
